...
import { Prisma, type party_games, type party_players} from '@prisma/client';
...
export class GameSession {
  private constructor(
    private game: party_games,
    private players: party_players[]
  ) {}

  // load a game session from database based off gameId. The promise promises a GameSession object returned or null if the gameId does not exist in the database. The GameSession object contains the game and players data.
  static async load(gameId: string): Promise<GameSession | null> {
    //access the db to see if games session exists with the gameId.
    const game = await db.party_games.findUnique({ where: { id: gameId } });
    //if it doesnt return null
    if (!game) return null;

    //get the players in the game session with the gameId. Use Promise.all to get the players and game data at the same time.
    const [players] = await Promise.all([
      db.party_players.findMany({ where: { game_id: gameId } })
    ]);
    //return a new GameSession object with the game and players data.
    return new GameSession(game, players);
  }
